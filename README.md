...
- 🚀 **Stage 2 – Migration execution** (parallel migrations, max 10 at a time)
...
- 🚀 **Stage 4 - Vars Migration**
- 🔐 **Stage 5 - Repository Rulesets Migration**
...
- Parallel migrations (up to 10 at a time).
...
### Stage 5: Repository Rulesets Migration
Migrates repository-level rulesets.
...
- Enforcement status
- Ruleset conditions
- Repository rule configurations
- Pull request rules
...
```
.github/workflows/ghe-ghec-with-vars-approver-gate.yml   # workflow, will create a UI based approval (pre → migrate → post → Vars & Env migration and repo reulsets)
...
scripts/
  ...
  6_repo-rulesets-migration.ps1  # Stage 5: Repository Rulesets migration
  6_repo-rulesets-migration.sh   # Stage 5: Repository Rulesets migration
...
4. Select stage toggles as required:
   ...
   - `run_repository_rulesets_migration`
...
### Repository Rulesets Migration 
- Execution status
- Repositories processed
- Repository rulesets migrated
- Repository rulesets skipped (already exists)
- Failed repository rulesets
- Ruleset migration warnings (if any)
...
- **Rulesets Migration Issues**: Review `rulesets_migration.log` for bypass actors, merge queue warnings, or unsupported configurations.
